...
# === SESSION STATE FOR MULTI-PAGE SCREEN FLOW ===
if "screen_view" not in st.session_state:
    st.session_state.screen_view = "menu_screen"  # Default screen block
...
st.sidebar.markdown("---")
st.sidebar.caption("Just Order v7.0 • Split Billing Protocol")


# ==================================================
# 🔑 MODE A: OWNER ADMIN DASHBOARD 
# ==================================================
if app_mode == "🔑 Owner Admin Dashboard":
    ...
    if password == "suraj123":
        ...
        tab_add_shop, tab_add_item, tab_edit_item, tab_view_orders = st.tabs(["➕ Add New Shop", "📸 Upload New Items", "✏️ Change Item Name/Price", "📊 View Orders"])
            ...
            if orders_rows:
                for row in orders_rows:
                    ...
                    with st.expander(f"📦 Order #{o_id} for {d_to} - Total: ₹{amt}"):
                        st.write(f"**Ordered By:** {o_by} | **Address:** {addr}")
                        phone_num = d_to.split("-")[-1].strip() if "-" in d_to else "0000000000"
                        manual_wa_url = generate_whatsapp_link(phone_num, d_to, "Hand-delivered Item Check", amt-30, 30, amt, "✅ Paid to Rider")
                        st.markdown(f'<a href="{manual_wa_url}" target="_blank" style="background-color:#007bff; color:white; padding:6px 12px; text-decoration:none; border-radius:5px; font-weight:bold; display:inline-block;">🟢 Send Cash Bill on WhatsApp</a>', unsafe_allow_html=True)
            conn.close()
...
# ==========================================
    ...
    if st.session_state.screen_view == "billing_screen":
        ...
        # Displaying the clean final invoice details
        for item_name, info in st.session_state.global_cart.items():
            ...
            st.write(f"🔹 **{item_name}** x  {info['qty']} Pcs = **₹{cost}** <span style='color:#a0a0a0; font-size:12px;'>({info['type']})</span>", unsafe_allow_html=True)
        ...
        with c_bill2:
            st.subheader("🛡️ Quality Safety Check")
            open_box_delivery = st.checkbox("📦 Request Open Box Delivery (Check package before accepting!)")

        st.write("---")
        st.subheader("📍 Contact & Address Registry")
        
        # Mandatory Inputs with Clean Custom Placeholders
        ...
        if is_gift:
            person_b_name = st.text_input("Receiver Full Name *", placeholder="Your Name")
            person_b_phone = st.text_input("Receiver 10-Digit Phone *", placeholder="Your Phone Number")
            address_landmark = st.text_area("Receiver Delivery Address & Landmarks *", placeholder="Your Address")
            person_final_name = f"{person_b_name} - {person_b_phone}"
            phone_to_send = person_b_phone.strip()
        else:
            person_final_name = f"{person_a_name} - {person_a_phone}"
            phone_to_send = person_a_phone.strip()

        st.write("---")
        payment_choice = st.radio("Select Payment Mode:", ["UPI Online Instant Pay", "Cash on Delivery (COD)"])
        
        # 🌟 EXPLICIT UPDATE: LINKING YOUR BRAND NEW CUSTOM UPI ID HERE
        if payment_choice == "UPI Online Instant Pay":
            upi_url = f"upi://pay?pa=suraj.u7870-1@oksbi&pn=Just+Order&am={grand_total}&cu=INR"
            st.markdown(f'<a href="{upi_url}" style="background-color:#28a745; color:white; padding:14px 28px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; font-size:16px; box-shadow: 0px 4px 10px rgba(40,167,69,0.3);">🚀 Pay ₹{grand_total} Now via UPI</a>', unsafe_allow_html=True)
            
        st.markdown("<br><br>", unsafe_allow_html=True)
        
        # Final checkout execution trigger
        if st.button("🏁 Confirm & Place Final Just Order", type="primary", use_container_width=True):
            if not person_a_name.strip() or not person_a_phone.strip() or not address_landmark.strip() or (is_gift and (not person_b_name.strip() or not person_b_phone.strip())):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (f"{person_a_name} - {person_a_phone}", person_final_name, address_landmark, grand_total, st.session_state.service_tag, open_box_flag, pay_status_db, time_stamp))
                conn.commit()
                conn.close()
                ...
                # Digital WhatsApp invoice redirection dispatch
                auto_wa_url = generate_whatsapp_link(phone_to_send, person_final_name, cart_summary_text, total_items_price, delivery_fee, grand_total, "✅ Paid via UPI Online" if payment_choice == "UPI Online Instant Pay" else "⏳ COD Pending")
                st.markdown(f'<a href="{auto_wa_url}" target="_blank" style="background-color:#25D366; color:white; padding:12px 24px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; margin-top:15px;">💬 Open WhatsApp & Send Digital Bill Instantly</a>', unsafe_allow_html=True)
                
                # Clear session state data post-ordering
                st.session_state.global_cart = {}
    ...
    # --------------------------------------------------
        ...
        # 🌟 REDIRECT TRIGGER COMPONENT FOR DEDICATED BILLING PAGE
        if merged_cart:
            st.markdown("---")
            st.info(f"🛒 Aapne **{len(merged_cart)}** types ke items select kiye hain.")
            ...
            if st.button("🚀 Proceed to Checkout / View Bill Summary", type="primary", use_container_width=True):
                ...
                st.session_state.screen_view = "billing_screen"  # Triggers page shift
                st.rerun()

    # Shared Marketing Panel at the end of customer journeys
    st.write("---")
